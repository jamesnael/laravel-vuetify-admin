...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (Auth::attempt(['username' => $request->input('username'), 'password' => $request->input('password')], $remember)) {
            ...
            if (in_array('auth.login', Auth::user()->jabatan ? Auth::user()->jabatan->hak_akses : [])) {
                return response_json(true, null, 'Login Berhasil.', redirect()->intended($this->redirectPath)->getTargetUrl());
            } else {
                Auth::logout();
                return response_json(false, 'UserNotFoundException', 'Identitas tersebut tidak mendapatkan hak akses untuk masuk.');
            }
        }
